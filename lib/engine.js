...
function renderPage(pageName, options, callback) {
    ...
    try {
        ...
        for (var i = 0; i < syncAssets.length; i++) {
            ...
            var type = list.shift();
            fnList.push(getAssets(type, list.join(','), syncAssets[i].tag, cfg));
        }
...
function getPageInfo(pageName, options, cfg, fileData) {
    ...
    if (cfg.cache == false || checkExpire(filePath, cfg)) {
        ...
        if (cfg.autoAsset) {
            ...
            //获取页面js
            console.log(123);
            componentJSList.push('tpls/' + nodePath.basename(filePath, '.html'));
            ...
            //在结尾插入引用JS
            pageObj.code = pageObj.code.replace(/<\/body>/ig, '{{asset "script' + script + '\n<\/body>');
            //在头部尾插入引用css
            pageObj.code = pageObj.code.replace(/<\/head>/ig, '{{asset "css' + script + '\n<\/head>');
        }

    ...
    try {
        ...
        Promise.all(fnList).then(function(result) {
            if (result) {
                ...
                callback(null, require('html').prettyPrint(tempTpl));
            }
...
function getPageInfo(pageName, options, cfg, fileData) {
    ...
    var openTag = options.settings.$config.openTag;
    var closeTag = options.settings.$config.closeTag;
    ...
    if (cfg.cache == false || checkExpire(filePath, cfg)) {
        ...
        if (cfg.autoAsset) {
            ...
            if (componentJSList.length > 0) {
                ...
                script = script + '" $$tpldata'+closeTag;
            }
            //var jsList = getComponentJs(pageName);
            //在结尾插入引用JS

            pageObj.code = pageObj.code.replace(new RegExp(openTag+'asset "script"'+closeTag, 'ig'), openTag+'asset "script' + script);
            //在头部尾插入引用css
        }
        // else{
        //     pageObj.code = pageObj.code.replace(new RegExp(openTag+'asset "script"'+closeTag, 'ig'), '<!-- 需开启 autoAsset:asset "script" -->');

        // }
        pageObj.code = pageObj.code.replace(new RegExp(openTag+'asset .*?'+closeTag, 'ig'), openTag+'asset "script' + script);
...
function preCompile(code, cfg) {
        ...
        code = code.replace(reg_module, function(fullCode, modstr, modName, attrs, modcontent) {
            ...
            if (modCode) {
                ...
                if (reg_module_attrs.test(attrs)) {
                    ...
                    if (attrHash.scope) {
                        hasScope = true;
                        retStr += openTag+'var ' + attrHash.scope + closeTag;
                    }
                }
                // 替换插入点
                ...
                if (hasScope) {
                    retStr = retStr + openTag+'/var'+ closeTag;
                }
                return retStr;
            } else {
    ...
    code = code.replace(/<scope:(\S+=\S+)>/ig, function(a, c) {
        // scopes.push(c);
        return  openTag + 'var ' + c + closeTag
    });
    code = code.replace(/<\/scope>/ig, openTag + '/var'+ closeTag);
    //
...
function processImagePath(code, type, name, cfg) {
    ...
    if (m_dir_hash[type]) {
        ...
        */
        code = code.replace(/\$res\((!?.)([^\'\"]+)\1\)/g,
            function(a,str, imgpath) {
